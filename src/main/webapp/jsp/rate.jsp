...
<nav class="navbar navbar-expand-lg navbar-shadow bg-light">
    <div class="ms-auto me-auto" id="navbarNav">
        <ul class="navbar-nav">
            <li class="nav-item">
                <a class="nav-link" href="/SemesterWork/users?user=${user}">Main</a>
            </li>
            <li class="nav-item">
                <a class="nav-link active" href="/SemesterWork/users/skills?user=${user}"><u>Skills</u></a>
            </li>
        </ul>
    </div>
</nav>
...
<div class="container mt-4">
    <div class="skill-container card p-5">
        <div class="skill-mini-text" >
            <a href="/SemesterWork/users/skills?user=${user}&skill=${skill}" style="color: darkgray">Back</a>
        </div>
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="custom-font-bold">Reviews</h1>
        </div>
        <c:choose>
            <c:when test="${not empty requestScope.rates}">
                <c:forEach var="rate" items="${requestScope.rates}">
                    <c:set var="curColor" value="color-${rate.id}" />
                    <div class="skill-container">
                        <div class="skill-header" style="background-color: ${requestScope[curColor]}">
                                ${rate.user.username}
                            <div class="ms-auto">${rate.rate}★</div>
                        </div>
