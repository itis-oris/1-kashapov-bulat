...
<div class="container mt-4">
    <div class="skill-container card p-5">
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="custom-font-bold">Users</h1>
        </div>

        <c:choose>
            <c:when test="${not empty requestScope.users}">
                <c:forEach var="user" items="${users}">
                    <div class="skill-container" style="height: 100px">
                        <a href="/SemesterWork/users?user=${user.username}">
                            <div class="skill-header black-color">
                                <img src="http://localhost:8080/SemesterWork/${user.avatar}" style="height:40px" alt="User Avatar" class="rounded-circle mb-3 black-color">
                                    ${user.username}
                            </div>
                            <div class="skill-body skill-mini-text">
                                <p>SkillPoints: ${user.skillpoints}</p>
                            </div>
                        </a>
                    </div>
                </c:forEach>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center mb-0">
                    <li class="page-item ${requestScope.page == 1 ? 'disabled' : ''}">
                        <a class="black-color page-link" href="http://localhost:8080/SemesterWork/users?page=${page - 1}">&laquo;</a>
                    </li>
                    <c:forEach begin="1" end="${numOfPages}" var="i">
                        <li class="page-item">
                            <a class="page-link custom-active ${i == page ? 'active' : ''}" href="http://localhost:8080/SemesterWork/users?page=${i}">
                                    ${i}
                            </a>
                        </li>
                    </c:forEach>
                    <li class="page-item ${requestScope.page == numOfPages ? 'disabled' : ''}">
                        <a class="black-color page-link" href="http://localhost:8080/SemesterWork/users?page=${page + 1}">&raquo;</a>
                    </li>
                </ul>
            </nav>
        </div>
